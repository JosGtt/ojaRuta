...
-- Ejecutar: psql -U tu_usuario -d tu_base_de_datos -f 007_reestructurar_tabla_envios_pg96.sql
...
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'envios') THEN
        ...
        RAISE NOTICE 'Datos migrados exitosamente desde la tabla envios existente';
    ELSE
        RAISE NOTICE 'No existe tabla envios anterior - creando tabla nueva';
    END IF;
END $$;
...
BEGIN
    ...
    IF NEW.estado = 'enviado' AND OLD.estado != 'enviado' AND NEW.hoja_id IS NOT NULL THEN
        ...
        INSERT INTO seguimiento (
            ...
            NEW.hoja_id,
            NEW.destino_id,
            'Envío marcado como enviado automáticamente. Destinatario: ' || NEW.destinatario_nombre,
            NEW.usuario_id,
            'finalizado',
            NEW.fecha_envio
        );
...
-- 8. Trigger para actualizar estado de hoja automáticamente (PostgreSQL 9.6)
...
SELECT 'Tabla envios reestructurada correctamente para PostgreSQL 9.6' AS resultado;
