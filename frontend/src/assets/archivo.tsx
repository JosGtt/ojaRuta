import React from "react";

interface IconProps extends React.SVGProps<SVGSVGElement> {
  width?: number;
  height?: number;
  fill?: string;
}

const ArchivoIcon: React.FC<IconProps> = ({ width = 24, height = 24, fill = "#fff", ...props }) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    width={width}
    height={height}
    fill={fill}
    viewBox="0 0 60 60"
    {...props}
  >
    <g>
      <path 
        fill={fill} 
        d="M57.49 27H54v-6.268A2.736 2.736 0 0 0 51.268 18H50V0H10v2H7v2H4v7H2.732A2.736 2.736 0 0 0 0 13.732v43.687h.006c-.005.563.17 1.114.522 1.575A2.546 2.546 0 0 0 2.565 60h44.759c1.157 0 2.175-.78 2.449-1.813L60 30.149v-.177C60 28.25 58.944 27 57.49 27zm-6.222-7c.404 0 .732.328.732.732V27h-2v-7h1.268zM48 2v25H12.731a2.56 2.56 0 0 0-.674.098c-.018.005-.039.003-.057.008V2h36zM10 4v25.585l-.063.173L9 32.326V4h1zM6 6h1v31.81l-1 2.741V6zm-4 7.732c0-.404.329-.732.732-.732H4v32.943l-2 5.455V13.732zm45.868 43.852a.562.562 0 0 1-.544.416H2.565a.552.552 0 0 1-.448-.222c-.063-.082-.16-.256-.123-.408L4 51.87v.001l3-8.225 3-8.225v.003l1.932-5.301.068-.185.16-.439.026-.082a.566.566 0 0 1 .545-.417H57.49c.379 0 .477.546.501.819L47.868 57.584z" 
      />
      <path 
        fill={fill} 
        d="M18 17h24a1 1 0 1 0 0-2H18a1 1 0 1 0 0 2zM18 10h10a1 1 0 1 0 0-2H18a1 1 0 1 0 0 2zM18 24h24a1 1 0 1 0 0-2H18a1 1 0 1 0 0 2z" 
      />
    </g>
  </svg>
);

export default ArchivoIcon;